...
struct JSONArray: Codable, Content {
    let inList: [Post]
}

struct ErorrMessage: Codable, Content {
    let message: String
}

struct OutList: Codable, Content{
    var outList:[String]
    
    
    }

struct Post: Content,Equatable,Comparable {
    static func < (lhs: Post, rhs: Post) -> Bool {
        
        guard let left = lhs.pri
            else {
                return false
        }
        
        guard let right = rhs.pri
            else{
                return false
        }
...
public struct PriorityQueue<T> {
    ...
    public mutating func allDeque()-> [T]?{
        var array = [T]()
...
public func routes(_ router: Router) throws  {
        router.post("queue") { req -> OutList in
            ...
            for elements in postArray.indices{
                if(postArray[elements].cmd == "enqueue"){
                queue.enqueue(postArray[elements])
                }else if(postArray[elements].cmd == "dequeue"){
                    queue.dequeue()
                }
            }
